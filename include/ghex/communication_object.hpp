...
class communication_object
{
  ...
  private: // implementation
    ...
    exchange_u(Iterator first, Iterator last)
    {
        if (std::string(m_comm.get_transport_option("name")) == "nccl")
        {
            throw std::runtime_error(
                "GHEX: optimized regular-grid GPU exchange is not supported with NCCL backend");
        }
  ...
  private: // wait functions
    ...
    void wait()
    {
        // TODO: This function has a big overlap with `is_ready()` should it be implemented
